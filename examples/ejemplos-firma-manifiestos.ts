/**
 * Ejemplo: firma de manifiesto vía POST /api/v4/manifests usando el SDK de FiscalAPI.
 * El endpoint recibe el certificado (.cer), la llave privada (.key) y la contraseña en base64,
 * y devuelve el PDF del manifiesto firmado (también en base64).
 */

import {
  ApiResponse,
  FiscalapiClient,
  FiscalapiSettings,
  IFiscalapiClient,
  SignManifestRequest,
  SignManifestResponse
} from '../src/index';
import { inspect } from 'util';
import * as fs from 'fs';
import * as path from 'path';

// Configuración de la consola para mostrar objetos anidados
inspect.defaultOptions.depth = null;
inspect.defaultOptions.colors = true;

// Configuración de FiscalAPI
const settings: FiscalapiSettings = {
    apiUrl: 'https://test.fiscalapi.com', // https://live.fiscalapi.com
    apiKey: '<API_KEY>', // API key de FiscalAPI
    tenant: '<TENANT_KEY>', // Tenant de FiscalAPI
    debug: true // true, imprime raw request y response en consola, util durante el desarrollo de la integración.
};

// Carpeta donde se guarda el PDF firmado que devuelve el endpoint
const carpetaSalida: string = 'C:/facturas';

// Sellos SAT de prueba (ESCUELA KEMPER URGATE)
const escuelaKemperUrgateBase64CerFIEL = "";
const escuelaKemperUrgateBase64KeyFIEL = "";
const password = "12345678a";

/**
 * Guarda en disco el archivo devuelto por el endpoint.
 * El contenido es binario, por lo que se decodifica a Buffer y no a texto.
 */
function guardarArchivoFirmado(archivo: SignManifestResponse): string {
  if (!fs.existsSync(carpetaSalida)) {
    fs.mkdirSync(carpetaSalida, { recursive: true });
  }

  const rutaArchivo: string = path.join(carpetaSalida, `${archivo.fileName}`);
  fs.writeFileSync(rutaArchivo, Buffer.from(archivo.base64File, 'base64'));
  return rutaArchivo;
}

// ============================================================================
// 1. FIRMA DE CARTA MANIFIESTO (ESCUELA KEMPER URGATE)
// ============================================================================
async function firmaCartaManifiesto(client: IFiscalapiClient): Promise<void> {
  console.log('\n=== Firma de Carta Manifiesto (ESCUELA KEMPER URGATE) ===\n');

  const request: SignManifestRequest = {
    base64Cer: escuelaKemperUrgateBase64CerFIEL,
    base64Key: escuelaKemperUrgateBase64KeyFIEL,
    password: password
  };

  const response: ApiResponse<SignManifestResponse> = await client.manifests.sign(request);
  console.log('Response:', response);

  if (response.succeeded) {
    const manifiesto: SignManifestResponse = response.data;
    console.log('Nombre:', manifiesto.fileName);
    console.log('Extensión:', manifiesto.fileExtension);
    console.log('Manifiesto guardado en:', guardarArchivoFirmado(manifiesto));
  }
}

// ============================================================================
// FUNCION PRINCIPAL
// ============================================================================
async function main(): Promise<void> {
  console.log('=== Ejemplos de Firma de Manifiesto FiscalAPI ===\n');

  const client: IFiscalapiClient = FiscalapiClient.create(settings);

  try {
    // Comentar los casos de uso que no se desean ejecutar

     await firmaCartaManifiesto(client);

    console.log('\nEjecución completada.');
  } catch (error) {
    console.error('Error:', error);
  }
}

// Ejecutar función principal
main();
